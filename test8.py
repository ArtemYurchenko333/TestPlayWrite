...
from proxy_auth22 import proxies, get_random_proxy, proxy_info

class MultiUserUhmegleBot:
    def __init__(self, max_users=15):
    ...
    async def run(self):
        ...
        try:
            ...
            # 1. Тестируем TCP соединение с прокси
            print("\n1️⃣ ТЕСТ TCP СОЕДИНЕНИЯ")
            print("-" * 40)
            if not self.test_proxy_connection():
                print("⚠️ Прокси сервер недоступен, но продолжаем...")
            
            # 2. Проверяем прокси через requests
            print("\n2️⃣ ТЕСТ ПРОКСИ ЧЕРЕЗ REQUESTS")
            print("-" * 40)
            if not self.test_proxy_with_requests():
                print("⚠️ Прокси не работает через requests, но продолжаем...")
            
            # 3. Настраиваем браузер
            print("\n3️⃣ НАСТРОЙКА БРАУЗЕРА")
            print("-" * 40)
            if not await self.setup_browser_with_proxy():
                print("❌ Не удалось настроить браузер")
                return False
            
            # 4. Проверяем IP в браузере
            print("\n4️⃣ ПРОВЕРКА IP В БРАУЗЕРЕ")
            print("-" * 40)
            ip_check_result = await self.check_ip_in_browser()
            if not ip_check_result:
                print("❌ Серьезные проблемы с прокси!")
                self.diagnose_proxy_issues()
                
                # Даем пользователю выбор
                print("\n❓ Хотите продолжить несмотря на проблемы с прокси?")
                print("1. Да, продолжить (может работать без прокси)")
                print("2. Нет, остановить выполнение")
                
                try:
                    choice = input("Выберите (1-2): ").strip()
                    if choice != "1":
                        print("⏹️ Выполнение остановлено пользователем")
                        return False
                except:
                    print("⏹️ Выполнение остановлено")
                    return False
                    
                print("⚠️ Продолжаем с возможными проблемами прокси...")
            
            # 5. Открываем сайт с повторными попытками
            print("\n5️⃣ ОТКРЫТИЕ САЙТА")
            print("-" * 40)
            ...
            # 6. Ждем подключения к серверу и ищем кнопку Start
            print("\n6️⃣ ПОДКЛЮЧЕНИЕ К СЕРВЕРУ")
            print("-" * 40)
            if not await self.wait_for_connection_and_start():
                print("❌ Не удалось подключиться к серверу или найти кнопку Start")
                return False
            
            # 7. Ждем готовности чата
            print("\n7️⃣ ОЖИДАНИЕ ГОТОВНОСТИ ЧАТА")
            print("-" * 40)
            if not await self.wait_for_chat_ready():
                print("⚠️ Чат может быть не готов, но продолжаем...")
            
            # 8. Обрабатываем всех пользователей
            print("\n8️⃣ ОБРАБОТКА ПОЛЬЗОВАТЕЛЕЙ")
            print("-" * 40)
...
async def main():
    ...
    print("=" * 60)
    print("🔧 Функции:")
    print("   ✅ Отправка сообщений 15 пользователям")
    ...
    try:
        ...
        max_users = input("Введите количество пользователей (по умолчанию 15): ").strip()
